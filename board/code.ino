...
void loadConfig()
{
  ...
  {
    CORRECT_PASSWORD = p;
    Serial.println("[CFG] PIN loaded from flash.");
  }
  ...
  {
    TELEGRAM_TOKEN = tok;
    Serial.println("[CFG] Telegram token loaded.");
  }
  if (cid.length() > 0)
  {
    TELEGRAM_CHAT_ID = cid;
    Serial.println("[CFG] Telegram chatId loaded.");
  }
...
void processKey(char key)
{
  ...
  if (key == 'B')
  {
    if (alertTriggered || vibAlertTriggered)
      resetAlert();
    enteredPassword = "";
    return;
  }
