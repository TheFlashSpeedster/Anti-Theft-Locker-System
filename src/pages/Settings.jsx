...
// ── Main Component ────────────────────────────────────────────────────────────
export default function Settings({ mode = 'live' }) {
  // ── Telegram ───────────────────────────────────────────────────────────────
  ...
  const [buzzHwStatus,  setBuzzHwStatus]  = useState('idle'); // idle | saving | ok | error

  // Fetch current vib sensor and buzzer states on mount
  useEffect(() => {
    if (mode === 'test') {
      try {
        const ts = JSON.parse(localStorage.getItem('test_state')) || {};
        setVibEnabled(ts.vibSensorEnabled ?? false);
        setBuzzEnabled(ts.buzzerEnabled ?? true);
      } catch {}
      return;
    }
    fetch(`${ESP32_IP}/status`, { signal: AbortSignal.timeout(2000) })
      .then(r => r.json())
      .then(d => {
        if (typeof d.vibSensorEnabled === 'boolean') setVibEnabled(d.vibSensorEnabled);
        if (typeof d.buzzerEnabled === 'boolean') setBuzzEnabled(d.buzzerEnabled);
      })
      .catch(() => {});
  }, [mode]);
  ...
  const toggleVibSensor = async () => {
    ...
    if (mode === 'test') {
      try {
        const ts = JSON.parse(localStorage.getItem('test_state')) || {};
        ts.vibSensorEnabled = next;
        localStorage.setItem('test_state', JSON.stringify(ts));
        setVibEnabled(next);
        setVibHwStatus('ok');
      } catch {}
      return;
  ...
  const toggleBuzzer = async () => {
    const next = !buzzEnabled;
    if (mode === 'test') {
      try {
        const ts = JSON.parse(localStorage.getItem('test_state')) || {};
        ts.buzzerEnabled = next;
        localStorage.setItem('test_state', JSON.stringify(ts));
        setBuzzEnabled(next);
        setBuzzHwStatus('ok');
      } catch {}
    ...
    setBuzzHwStatus('saving');
    try {
      const res = await fetch(`${ESP32_IP}/config`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ buzzerEnabled: next }),
        signal: AbortSignal.timeout(3000),
      });
      if (!res.ok) throw new Error();
      setBuzzEnabled(next);
  ...
  useEffect(() => { if (buzzHwStatus !== 'idle') { const t = setTimeout(() => setBuzzHwStatus('idle'), 3500); return () => clearTimeout(t); } }, [buzzHwStatus]);
  useEffect(() => { if (vibHwStatus  !== 'idle') { const t = setTimeout(() => setVibHwStatus('idle'),  3500); return () => clearTimeout(t); } }, [vibHwStatus]);
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="glass-panel p-6 rounded-2xl relative overflow-hidden">
        <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl"
          style={{ background: vibEnabled || buzzEnabled ? 'var(--color-secondary)' : '#6b7280' }} />

        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-secondary">developer_board</span>
            <div>
              <h2 className="font-manrope text-xl font-bold tracking-wider uppercase">Hardware Configuration</h2>
              <p className="text-xs text-text-variant font-mono mt-0.5">Configure physical sensors and alert peripherals on the ESP32.</p>
            </div>
          </div>
          <div className="flex gap-2">
            <SaveBanner status={vibHwStatus} />
            <SaveBanner status={buzzHwStatus} />
          </div>
        </div>
        ...
        {/* Buzzer toggle row */}
        <div className={`mt-3 flex items-center justify-between gap-4 p-4 rounded-xl border transition-all
          ${buzzEnabled ? 'bg-secondary/5 border-secondary/20' : 'bg-white/3 border-white/8'}`}>
          <div className="flex items-center gap-3">
            <span className={`material-symbols-outlined text-xl ${buzzEnabled ? 'text-secondary' : 'text-text-variant/40'}`}>campaign</span>
            <div>
              <p className="text-sm font-bold font-manrope">Audible Buzzer Alarm</p>
              <p className="text-[10px] font-mono text-text-variant/60 mt-0.5">
                {buzzEnabled
                  ? 'GPIO 2 active — buzzer sound will trigger during security breaches'
                  : 'GPIO 2 muted — silent alarm mode, only status LED will flash during breaches'}
              </p>
            </div>
          </div>
          <button onClick={toggleBuzzer} disabled={buzzHwStatus === 'saving'}
            className="flex-shrink-0 relative w-14 h-7 rounded-full border-2 transition-all duration-300 focus:outline-none disabled:opacity-50"
            style={{ borderColor: buzzEnabled ? 'rgba(0,240,180,0.5)' : 'rgba(255,255,255,0.15)' }}>
            <span className="absolute inset-0 rounded-full transition-all duration-300"
              style={{ background: buzzEnabled ? 'rgba(0,240,180,0.2)' : 'rgba(255,255,255,0.05)' }} />
            <span className="absolute top-0.5 w-5 h-5 rounded-full shadow-lg transition-all duration-300"
              style={{
                left: buzzEnabled ? 'calc(100% - 1.375rem)' : '0.125rem',
                background: buzzEnabled ? 'var(--color-secondary)' : '#6b7280',
              }} />
          </button>
        </div>

        <p className="text-[10px] font-mono text-text-variant/40 mt-3">
          ⚠ Settings are saved directly to the physical ESP32 flash memory. 
          Disabling the buzzer sound retains full visual alerts (flashing screen + LED) and Telegram notifications.
        </p>
      </div>
